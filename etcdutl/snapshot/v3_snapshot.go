...
func (s *v3Manager) Restore(cfg RestoreConfig) error {
	...
	if cfg.MarkCompacted || cfg.RevisionBump > 0 {
		if err = s.maybeModifyLatestRevision(cfg.MarkCompacted, cfg.RevisionBump); err != nil {
			return err
		}
	}
...
// maybeModifyLatestRevision can increase the latest revision by the given amount and sets the scheduled compaction
// to that revision so that the server will compact the database up to that revision.
func (s *v3Manager) maybeModifyLatestRevision(markCompacted bool, bumpAmount int64) error {
	be := backend.NewDefaultBackend(s.lg, s.outDbPath())
	defer be.Close()
	...
	if bumpAmount > 0 {
		latest = s.unsafeBumpRevision(tx, latest, bumpAmount)
	}
